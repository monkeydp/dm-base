package com.monkeydp.daios.dm.base.helper

import com.monkeydp.daios.dm.base.config.ComponentConfig
 ...
 */
object KodeinHelper {
    fun initKodein(config: ComponentConfig, vararg modules: Kodein.Module) =
            Kodein {
    ...
    private fun Kodein.MainBuilder.registerAll(annotKClass: KClass<out Annotation>, components: Collection<Any>) {
        ...
        kodeinComponent.registerItems.forEach {
            when (it) {
                KodeinComponent.RegisterItem.COMPONENT -> registerComponents(components)
                KodeinComponent.RegisterItem.LIST -> registerList(annotKClass, components)
                KodeinComponent.RegisterItem.SET -> registerSet(annotKClass, components)
                KodeinComponent.RegisterItem.MAP -> registerMap(annotKClass, components)
                else -> return@forEach
            }
        }
    }
